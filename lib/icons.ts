...
export const getTechIcon = (techName: string) => {
    ...
    if (normalized.includes("react") || normalized.includes("frontend")) return Globe;
    ...
    if (normalized.includes("security") || normalized.includes("encryption")) return Shield;
    if (normalized.includes("database") || normalized.includes("sql")) return Database;
    if (normalized.includes("docker") || normalized.includes("kubernetes")) return Box;
    if (normalized.includes("aws") || normalized.includes("cloud")) return Server;
    if (normalized.includes("linux") || normalized.includes("ubuntu")) return Terminal;
