...
export function linkifyTech(text: string): string {
    ...
    for (const tech of sortedTechs) {
        ...
        result = result.replace(
            regex,
            `<a href="${url}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 px-1.5 py-px rounded bg-blue-600 dark:bg-blue-500 text-white dark:text-white font-black hover:bg-blue-700 dark:hover:bg-blue-400 transition-colors group/link mx-0.5 text-xs uppercase tracking-wider decoration-0">${tech}<span class="inline-block transform transition-transform group-hover/link:-translate-y-0.5 group-hover/link:translate-x-0.5"><svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="opacity-70 group-hover/link:opacity-100 text-white"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg></span></a>`
        );
    }
    return result;
}
