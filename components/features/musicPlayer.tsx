...
export function MusicToggleButton() {
    ...
    return (
        <button
            onClick={toggleMusicPlayer}
            className="fixed bottom-8 right-8 z-[101] w-12 h-12 bg-black dark:bg-white text-white dark:text-black rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 group"
            title="Open Music Player"
        >
            <div className={`relative ${isPlaying ? 'animate-pulse' : ''}`}>
                <Music size={22} className={isPlaying ? 'text-green-500' : ''} />
                {isPlaying && (
